...
// Shared lock: only ONE screen capture (mushroom bg OR magnifier) may run at a time.
...
(function animate() {
  ...
  // Keep status panel anchored to top-right of mushroom
  if (statusPanel) {
...
(async () => {
  try {
    ...
    // Start click-through; pollMouse will enable cursor events only over the mushroom
    ...
    async function pollMouse() {
      try {
        ...
        const hit = window.__permOverlayActive || overMushroom || overPanel;
    ...
    // Drag the mushroom within the scene (not the whole window)

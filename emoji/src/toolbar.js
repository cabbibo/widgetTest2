...
async function pollCursor() {
  try {
    ...
    const over = inRect(pill) || (picker.classList.contains('open') && inRect(picker));
...
const collapse = () => { picker.classList.remove('open'); editingIndex = -1; renderPill(); appWindow.setSize(new LogicalSize(W, 82)); };
const expand   = () => appWindow.setSize(new LogicalSize(W, 380));
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && picker.classList.contains('open')) { e.preventDefault(); collapse(); }
});
// click anywhere outside the picker (empty area) closes it; clicking other apps blurs us
document.addEventListener('pointerdown', (e) => {
  if (!picker.classList.contains('open')) return;
  if (picker.contains(e.target) || pill.contains(e.target)) return;
  collapse();
});
window.addEventListener('blur', () => { if (picker.classList.contains('open')) collapse(); });
